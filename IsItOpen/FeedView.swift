...
struct FeedView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .refreshable {
                // Call the function to reload data when the user pulls to refresh
                viewModel.loadCheckIns()
            }
            .navigationBarTitle("Recent Check-Ins", displayMode: .inline)
            .onAppear {
                // Load check-ins directly without authentication check
                viewModel.loadCheckIns()
            }
        }
    }
...
struct CheckInCard: View {
    ...
    private func vote(up: Bool) {
        APIManager.postVote(checkInId: checkIn.id, up: up) { success in
            if success {
                print("Vote successful")
                viewModel.loadCheckIns() // Refresh the list after voting
            } else {
                print("Vote failed")
            }
        }
